...
public class SettingsEmailActivity extends Activity{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		txtMessage.setOnLongClickListener(null);


	    
	    
		dataAdapter = ArrayAdapter.createFromResource(SettingsEmailActivity.this, R.array.email_subjects_array,  R.layout.settings_email_list_item);
		dataAdapter.setDropDownViewResource(R.layout.settings_email_list_item);
		...
		if(Scale.getInstance().getUser() != null){
			txtName.setText(Scale.getInstance().getUser().getEmail());
		}else{
			txtName.setText("");
		}
		...
		buttonSend.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				String to = Scale.getInstance().getUser().getEmail();
				...
				if (to != null && to.length() == 0) {
					...
					Toast.makeText(SettingsEmailActivity.this,
							getString(R.string.try_to_send_mail),
							Toast.LENGTH_SHORT).show();
					...
					postMessageService.setOnTaskFinishedListener(new PostMessageFinishedListener() {
						...
						public void onTaskFinished(int responseCode) {
							if(responseCode == PostUtil.RETURN_OK){
								Toast.makeText(SettingsEmailActivity.this, getString(R.string.message_has_been_sent), Toast.LENGTH_LONG).show();
								finish();
							}else{
								Toast.makeText(SettingsEmailActivity.this, getString(R.string.message_could_not_be_send), Toast.LENGTH_LONG).show();
							}
						}
					});
